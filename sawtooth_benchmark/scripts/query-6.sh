...
do
	VALIDATED_TRANSACTIONS=$(( $(docker exec -t sawtooth-shell-default-0 sawtooth transaction list --url http://sawtooth-rest-api-default-0:8008 | wc -l) - 16 ))
	FLAG_WAIT_TIME=0
	if [ $VALIDATED_TRANSACTIONS -eq $PREVIOUS_TRANSACTION_NUMBER ]; then 
		TIME_CONTROL=$((TIME_CONTROL+1))
		FLAG_WAIT_TIME=1
		if [ $TIME_CONTROL -eq 75 ]; then
